...
- Three exceptions to that failure rule. ` + "`bmcp doctor`" + ` reports failing checks in its ordinary report on stdout with ` + "`\"ok\": false`" + ` and exits 1 — it is a diagnostic, so a failing check is its answer rather than an error. ` + "`--help`" + ` prints human text in every format; do not parse it. And ` + "`bmcp login`" + ` does not run under these formats at all: it answers ` + "`interactive_login_required`" + ` and exits 3, because it opens a browser and blocks. Adding ` + "`--verbose`" + ` also puts progress prose back on stderr, so do not combine it with ` + "`2>&1`" + `.

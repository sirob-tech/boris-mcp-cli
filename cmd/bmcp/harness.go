...
Every ` + "`bmcp doctor`" + ` reports a ` + "`credentials`" + ` row naming which credentials a call would use: an AWS profile and where that profile came from, or the credentials the environment carries and the profile they outrank. It ends in ` + "`— found, not verified`" + ` because it states what resolved locally and is not a verdict on whether those credentials work — nothing was contacted to form one. Only ` + "`bmcp doctor --deep`" + ` tests them, in its ` + "`auth`" + ` and ` + "`remote`" + ` rows, and both come from a single attempt against the server, so they cannot contradict each other. A failing ` + "`auth`" + ` row means either that bmcp never produced a signed request or that the BORIS gateway refused the one it did; every other remote failure leaves ` + "`auth`" + ` passing and fails ` + "`remote`" + ` alone.

If the ` + "`credentials`" + ` row names a source the user did not intend — stale ` + "`AWS_ACCESS_KEY_ID`" + ` in the environment displacing their configured profile, say — ` + "`bmcp --profile <name> <tool>`" + ` overrides it for one call, since a profile named for the invocation outranks anything ambient. One case bmcp corrects by itself: environment credentials stamped with a past ` + "`AWS_CREDENTIAL_EXPIRATION`" + ` — a lapsed ` + "`aws-vault exec`" + ` shell — are passed over in favour of the configured profile, and that row says so.
